...
contract('Contract', ([deployer, author]) => {
  let contract
  ...
  describe('videos', async () => {
    ...
    before(async () => {
      result = await contract.uploadVideo(hash, 'Video title', { from: author })
      videoCount = await contract.videoCount()
    })
    ...
    it('creates videos', async () => {
      ...
      await contract.uploadVideo('', 'Video title', { from: author }).should.be.rejected;

      // FAILURE: Video must have title
      await contract.uploadVideo('Video hash', '', { from: author }).should.be.rejected;
    })

    //check from Struct
    it('lists videos', async () => {
      const video = await contract.videos(videoCount)
